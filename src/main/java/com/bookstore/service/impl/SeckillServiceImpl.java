...
public class SeckillServiceImpl implements SeckillService {
    ...
    private SeckillBuyResultVO doCreateOrder(Long userId, SeckillActivity activity, Address address) {
        Book book = bookMapper.selectById(activity.getBookId());
        if (book == null || (book.getDeleted() != null && book.getDeleted() == 1) || book.getStatus() != 1) {
            throw new BusinessException(ResultCode.BOOK_NOT_FOUND);
        }
        int stockAffected = bookMapper.update(null,
    ...
    public void payOrder(Long userId, String orderNo) {
        ...
        walletService.pay(userId, orderNo, order.getTotalAmount());
        ...
        bookMapper.update(null,
            new LambdaUpdateWrapper<Book>()
                ...
                .setSql("sales_count = sales_count + " + order.getQuantity())
        );
    }
    ...
    public PageResult<SeckillOrderVO> listMyOrders(Long userId, String status, Integer page, Integer size) {
        ...
        Map<Long, Book> bookMap = loadBookMap(p.getRecords());
    ...
    private Map<Long, Book> loadBookMap(List<SeckillOrder> orders) {
        if (orders.isEmpty()) {
            return new HashMap<>();
        }
        ...
        return books.stream().collect(Collectors.toMap(Book::getId, x -> x));
    }

    private SeckillOrderVO toVO(SeckillOrder o, Book book) {
